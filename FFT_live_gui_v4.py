...
FREQ_TOLERANCE = 0.02
MIN_PEAK_DB = -80
MIN_FREQ_HZ = 5.0        # Schutz gegen DC / sehr langsame Signale
...
class RTB2004GUI(ctk.CTk):
    ...
    def __init__(self):
        super().__init__()
        self.title("RTB2004 – Time + FFT (2 Perioden automatisch)")
    ...
    def _build_gui(self):
        ...
        self.ax_time.set_title("Zeitdomäne (2 Perioden automatisch)")
    ...
    def update_gui(self):
        if not self.data_queue.empty():
            ...
            # ===== Zeitbereich: 2 Perioden =====
            if f0 > MIN_FREQ_HZ:
                period = 1.0 / f0
                visible_time = min(2 * period, ACQ_TIME)
            else:
                visible_time = ACQ_TIME

            visible_samples = max(10, int(n * (visible_time / ACQ_TIME)))
            trace_vis = trace[:visible_samples]
            t_vis = t[:visible_samples]

            self.line_time.set_data(t_vis, trace_vis)
            self.ax_time.set_xlim(0, visible_time)
